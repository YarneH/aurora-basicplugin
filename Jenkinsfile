    ...
    stages {
            ...
            steps {
                script {
                    ...
                    if (env.BRANCH_NAME == 'master' || env.BRANCH_NAME == 'dev') {
                        ...
                        sh "./gradlew basicprocessor:testReleaseUnitTest --tests '*UnitTest'"
                    } else {
                        sh "./gradlew testDebugUnitTest --tests '*UnitTest'"
                        sh "./gradlew basicprocessor:testDebugUnitTest --tests '*UnitTest'"
                    }
                    ...
                    jacoco sourcePattern: '**/src/main/java/com/aurora', 
                        ...
                        exclusionPattern: '**/*Test*.class,  **/basicplugin/*.class, **/R.class, **/R$*.class, **/BuildConfig, **/src/test, **/src/androidTest'
                }
            }
            ...
            steps {
                script {
                    ...
                    if (env.BRANCH_NAME == 'master' || env.BRANCH_NAME == 'dev') {
                        ...
                        sh "./gradlew basicprocessor:testReleaseUnitTest --tests '*LongTest'"
                    } else {
                        sh "./gradlew testDebugUnitTest --tests '*LongTest'"
                        sh "./gradlew basicprocessor:testDebugUnitTest --tests '*LongTest'"
                    }
            ...
            steps {
                script {
                    ...
                    if (env.BRANCH_NAME == 'master' || env.BRANCH_NAME == 'dev') {
                        ...
                        sh "./gradlew basicprocessor:testReleaseUnitTest --tests '*IntegTest'"
                    } else {
                        sh "./gradlew testDebugUnitTest --tests '*IntegTest'"
                        sh "./gradlew basicprocessor:testDebugUnitTest --tests '*IntegTest'"
                    }
        ...
        stage('Javadoc') {
            ...
            steps {
                // Generate javadoc
                sh """
                javadoc -d /var/www/javadoc/basicplugin/app/${env.BRANCH_NAME} -sourcepath ${WORKSPACE}/app/src/main/java -subpackages com -private \
                ...
                sh """
                javadoc -d /var/www/javadoc/basicplugin/basicprocessor/${env.BRANCH_NAME} \
                -sourcepath ${WORKSPACE}/basicprocessor/src/main/java \
        ...
        stage('Deployment') {
            ...
            steps {
                script {
                    ...
                    mv app/build/outputs/apk/release/app-release.apk /var/www/javadoc/deploy/basicplugin.apk
                    """
                }
            }
            post {
...
def slack_error_build() {
    slack_report(false, ':x: Basicplugin could not be built.', null, 'Build')
}
...
def slack_report(boolean successful, String text, JSONArray fields, failedStage='') {
    ...
    attachment.put('title_link', 'https://github.ugent.be/Aurora/basicplugin/commit/' + commit)
    if (text != '') {
    ...
    slackSend(channel: '#basicplugin-builds', attachments: attachments.toString(), teamDomain: 'aurora1819', baseUrl: 'https://hooks.slack.com/services/', token: token)
}
